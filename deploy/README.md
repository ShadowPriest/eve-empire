...
## Все дистрибутивы одной командой

```powershell
.\deployuild-all.ps1
```

За один проход собирает в `dist
elease` три пакета и `SHA256SUMS`:

| файл | что внутри |
|---|---|
| `eve-empire-<v>-mikrotik.zip` | образ arm64 (`docker save`), `ros-eve.rsc` с секретами из `.env`/`.env.prod` (если они есть), README с порядком обновления. Без баз: для установки прода с нуля по-прежнему `package.ps1` |
| `eve-empire-<v>-windows-amd64.zip` | `eve-empire.exe`, `sdeimport.exe`, `.env.example`, `SETUP.md`, лицензия |
| `eve-empire_<v>_amd64.deb` | служба systemd для Debian/Ubuntu: бинарники в `/usr/bin`, настройки в `/etc/eve-empire/eve-empire.env` (conffile), базы в `/var/lib/eve-empire`, пользователь `eve-empire`. Инструкция ставится в `/usr/share/doc/eve-empire/SETUP.md` |

Docker, dpkg-deb и ar не нужны: все бинарники статические, образ пакует `cmd/mkimage`,
deb собирает `cmd/mkdeb` (ar + два tar.gz, скрипты сопровождения и unit из `deploy/debian/`,
переводы строк нормализуются к LF независимо от autocrlf). Версия по умолчанию
`0.1.<число коммитов>+g<sha>` (`.dirty`, если есть незакоммиченное), явно `-Version 0.2.0`.
Ключи: `-DebArch amd64,arm64`, `-NoMikrotik`, `-NoWindows`, `-NoDeb`, `-Disk`, `-Callback`.

Скрипты `.ps1` с кириллицей должны быть в UTF-8 **с BOM**: Windows PowerShell 5.1 без него
читает файл как ANSI и падает на разборе.
...
**Вход и сессии** (необязательные, значения по умолчанию подходят):

```
/container/envs/add list=eve key=SIGNUP       value=first
/container/envs/add list=eve key=SESSION_DAYS value=90
```

`SIGNUP` — кто может завести НОВЫЙ кабинет: `first` (по умолчанию) — только первый
вход в пустую базу, `open` — любой неизвестный персонаж, `closed` — никто,
`corp` — тот, кто состоит в корпорации, где есть персонаж администратора (первый
вход в пустую базу при этом работает как `first`). `SESSION_DAYS` — срок жизни
сессии (кука `eve_session`, HttpOnly, без Secure: в LAN сервис живёт по HTTP).

**Как войти в первый раз:** любым персонажем из уже добавленных — попадёшь в его
кабинет со всеми альтами. Первый вход в пустую базу создаёт кабинет администратора;
остальные альты добавляются изнутри, «Настройки» → «+ Добавить персонажа».
База, перенесённая с ПК, приезжает с готовым кабинетом (миграция отдала ей всех
персонажей) — заводить ничего не нужно, достаточно войти любым альтом.

**Проверь `EVE_SCOPES` в `/container/envs`.** Если она там осталась со старых
времён, она перекрывает полный пресет «Альт» на странице входа: кабинет запросит
ровно то, что в ней написано, и вкладки на недостающие права пропадут. Убрать:

```
/container/envs/print where key=EVE_SCOPES
/container/envs/remove [find key=EVE_SCOPES]
```

Узкий пресет «Производство» от этой переменной не зависит: его четыре права
зашиты в коде.
